...
double SeqLearner::get_distance(std::string a, std::string b){
	return 1.0*abs(a[0] - b[0]);
}

void SeqLearner::generate_alphabet(){
	// string s = "";
	// char first_char = 'a';
	for (int i = 0; i < alphabet_size;i++){
		// s.push_back('a' + i);
		alphabet.push_back(string(1,'a' + i));
		// s.clear();
	}
}
...
int SeqLearner::run (const char *in,
	...
	alphabet_size = _sax_alphabet_size;
	max_distance = _max_distance;

	gettimeofday(&t_origin, NULL);

	//cout << "Generating alphabet...";
	generate_alphabet();
	//cout << "done" << endl;
	//cout.flush();
	...
	// THACH: for re-training with different label (multiclass case)
	if (seed.empty()){
	...
	for (unsigned int itr = 0; itr < maxitr; ++itr) {
		...
		step_length_opt = (sum_best_beta_opt[rule.loc[0]] - sum_best_beta[rule.loc[0]]) / compute_xi(rule.dist[0]);
		...
		// TODO: THACH check the convergence formula
	...
	for (unsigned int docid = 0; docid < transaction.size(); ++docid) {
		...
		for (unsigned int pos = 0; pos < transaction[docid].size(); ++pos) {
			...
			if (!SNode::tokenType) {
				...
				for (const string similar_candidate:alphabet){
					double candidate_distance = get_distance(similar_candidate,unigram);
					// NOTE: limit the expansion
					if (candidate_distance <= 1.0 && candidate_distance <= max_distance){
						SNode & tmp = seed[similar_candidate];
						tmp.add (docid,pos,candidate_distance);
						tmp.next.clear ();
						tmp.ne = similar_candidate;
						tmp.prev = 0;
					}
				}
				unigram.clear();
...
long double SeqLearner::computeLossTerm(const double& beta, const double &y, long double &exp_fraction){
	if (objective == 0) { //SLR
		...
		if (-y * beta > 8000) {
			...
			return log(1 + exp(-y * beta)); // TODO : xi
		}
	} //end SLR
